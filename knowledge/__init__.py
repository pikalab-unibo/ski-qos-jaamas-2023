...
def generate_census_income_knowledge():
    data = pd.read_csv(DATA_PATH / 'census-income-data.csv')
    ...
    with open(PATH / "census-income.txt", "w") as text_file:
        text_file.write(pretty_theory(knowledge))
...
def get_poker_knowledge() -> list[Formula]:
    knowledge = _get_knowledge(str(PATH / 'poker.txt'))
